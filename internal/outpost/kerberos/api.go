	...
	for _, provider := range apiProviders {
		...
		instance := &ProviderInstance{
			...
			KDC: &kdc.Server{
				...
				MaxUDPWorkers:               int(provider.GetMaxUdpWorkers()),
				MaxTCPConnections:           int(provider.GetMaxTcpConnections()),
				TCPIdleTimeout:              time.Duration(provider.GetTcpIdleTimeout()) * time.Second,
				MaxDatagramReplySize:        int(provider.GetMaxDatagramReplySize()),
