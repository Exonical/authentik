...
To replace it, edit the provider and click the rotate icon next to **Shared secret**. The outpost picks up the new secret within a few seconds, and RADIUS clients still using the old one are rejected until you update them. See [Secret rotation](../../../sys-mgmt/secret-rotation.md) for the rest.
