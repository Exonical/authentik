...
Leave **Shared secret** empty on create and authentik generates one. The provider's overview page shows it behind a reveal toggle.

To replace it, edit the provider and click the rotate icon next to **Shared secret**. Rotating applies immediately, whether or not you save the form. The outpost picks up the new secret within a few seconds, and RADIUS clients still using the old one are rejected until you update them. Each rotation is recorded as a `secret_rotate` event.
...
- [User Logout](../../flows-stages/stages/user_logout/index.md)
- [User Login](../../flows-stages/stages/user_login/index.md)
- [Deny](../../flows-stages/stages/deny/index.md)
