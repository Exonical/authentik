...
To replace the client secret of a confidential client, edit the provider under **Applications** > **Providers** and click the rotate icon next to **Client Secret**. The new secret takes effect immediately, whether or not you save the form: clients still using the old secret are rejected, and if the provider has no signing key, ID tokens signed with the old secret no longer validate. The field shows the new secret, ready to copy, and the provider's overview page shows it as well. Each rotation is recorded as a `secret_rotate` event.
