...
The **Client ID** and **Client Secret** are generated by authentik when left empty. Both are shown on the provider's overview page afterwards, the secret behind a reveal toggle, with a copy button next to each.

## Rotate the client secret

To replace the client secret of a confidential client, open the provider's overview page under **Applications** > **Providers** and click the rotate icon next to **Client secret**. The new secret takes effect immediately: clients still using the old secret are rejected, and if the provider has no signing key, ID tokens signed with the old secret no longer validate. Reveal or copy the new secret from the same page and update every client. Each rotation is recorded as a `secret_rotate` event.
