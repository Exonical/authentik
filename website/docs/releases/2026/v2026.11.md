...
OAuth2 client secrets, RADIUS shared secrets, token keys, and agent enrollment tokens can now be replaced with a freshly generated value, from the Admin interface or the API. Rotating takes effect immediately and needs the same `change` permission as editing the object. See [Secret rotation](../../sys-mgmt/secret-rotation.md) for what breaks when you rotate.
