...
OAuth2 client secrets, RADIUS shared secrets, token keys, and agent enrollment tokens can now be replaced with a freshly generated value, from the Admin interface or the API. Rotating takes effect immediately and needs the same `change` permission as editing the object.

The Admin interface no longer generates these values in the browser. Leave the field empty on create and authentik generates it, in the Admin interface and the API alike. Generated values now come from the model defaults, so new RADIUS shared secrets are 40 characters where the Admin interface previously made 128. Clearing the client secret of a confidential OAuth2 provider is now rejected.
