...
### Require actions on next login :ak-enterprise

Administrators can require users to complete one or more actions at their next login, before a session is created, by setting the `goauthentik.io/user/next-actions` attribute. Typical actions include changing a password or configuring an authenticator. Required actions are carried out via flows, and are cleared automatically after the user completes them. A session that already exists is blocked, and cannot authorize applications, until its user completes the pending actions.

For more details, refer to the [next actions documentation](../../users-sources/user/next_actions.mdx).
