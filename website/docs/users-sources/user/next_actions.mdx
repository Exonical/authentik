...
When a login flow reaches the [User Login stage](../../add-secure-apps/flows-stages/stages/user_login/index.mdx), authentik runs the required flows in the order they are listed, before completing the login. After the user completes a flow, authentik removes it from the attribute so it is not run again.

If the user already has a session, for example one created before the actions were assigned, authentik blocks that session from everything except completing the actions and logging out, and sends the user into the required flows on their next request. A blocked session also cannot authorize applications, so pending actions cannot be side-stepped by signing in to an application with an existing session.
...
If a listed flow doesn't exist or its policies deny the user, the login fails until the attribute is corrected or removed. On deployments without an enterprise license, authentik ignores this attribute.
